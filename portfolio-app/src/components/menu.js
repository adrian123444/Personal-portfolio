...
import { FaHome, FaBars, FaTimes, FaProjectDiagram, FaUserAstronaut } from 'react-icons/fa';
...
const Menu = ({ handleScroll }) => {
  ...
  return (
    <div className="relative z-10">
      ...
      <div className={`fixed top-0 left-0 right-0 z-40 justify-center space-x-5 bg-lightBgBrighter dark:bg-gray-800 p-3 lg:flex hidden`}>
        <button
      ...
      <div
        className={`lg:hidden fixed top-0 left-0 right-0 z-40 flex flex-col items-center justify-center bg-lightBgBrighter dark:bg-gray-800 p-3 transition-all duration-[1200ms] ease-in-out ${mobileMenu ? 'transform translate-x-0 opacity-100' : 'transform translate-x-full opacity-0'}`}
      >
        <button
          onClick={() => handleScroll('home')}
          className="flex items-center p-3 rounded-full hover:animate-pulse"
